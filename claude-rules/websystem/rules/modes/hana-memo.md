...
## 0.5 目的と読み手（誰に何のために書くか）

**このメモは、開発エージェントがスタッフ側へ情報を届けるための橋渡しでもある。** 開発エージェントはスタッフやクライアントに直接届ける手段を持たないことが多く、共有メモがその実質的な唯一の経路になる。したがって：

- **読み手は非開発者**（社内スタッフ・案件担当者。案件によってはクライアントへ転記・転送されうる）。書く前に「これは誰が読むのか」を確認する。
- **本文は平易な言葉で「何が残っているか・何を待っているか」を書く。** 内部の提案番号（`R-NNN`・`PI-NNN` 等）や技術用語（構造化データ／JSON-LD、見出し階層、alt 属性、SCSS 等）はそのまま出さず、日常語に言い換える（例：「見出しの整理」「画像の読み上げ対応」）。判断に必要なら「誰が何をすればよいか」を添える。
- **正本の丸写しにしない。** 役割分担は次のとおり：

| | 置き場 | 読み手 | 粒度 |
|---|---|---|---|
| 正本 | `documents/pending-issues.md` 等（開発プロジェクト側） | 開発者 | 技術的詳細・経緯まで |
| 共有メモ | hana-tools プロジェクトメモ | スタッフ | 平易に噛み砕いた要約 |

> 手順としては「正本を先に更新 → それをスタッフ向けに噛み砕いて共有メモへ」。正本の記述をコピーして貼るのではなく、**読み手に合わせて書き直す**。
...
**共有メモには人手で書かれた記録が入っていることが多い**（担当者の打ち合わせメモ、クライアントとのやり取りの控えなど）。AI から見て構造が整っていなくても、**それは消す対象ではなく残す対象**。統合するときは既存の記述に手を入れず、自分の追記を別の見出しとして足す。既存部分を整理・要約したくなったら、勝手に直さずユーザーに提案する。
...
> **文字コード**：本文ファイルは **UTF-8（BOM 無し）** で扱う。シェル・OS によっては既定のエンコーディングで読み書きすると日本語が化けるため、送信前に本文を読み返して確認する。回避策が必要になった環境では、その手順を**各プロジェクトの環境メモ**（`.claude/rules/private.md` があればそこ、無ければ `project-config.md`）に記録して次回以降迷わないようにする（環境固有の回避策はこのルール本文に書かない）。
...
- 「経緯 / 未対応 / 今後の判断ポイント」のような項目名は `<strong>` + `<p>` で書く（`documents/pending-issues.md` の記録フォーマットと揃う）。**揃えるのは構成だけで、中身はスタッフ向けの平易な言葉に書き直す**（§0.5）。
...
- **読み手は非開発者。** 正本を丸写しせず、平易な言葉で「何が残っているか・何を待っているか」に書き直す（§0.5）。
- **人手で書かれた既存の記録（打ち合わせメモ等）には手を入れない。** 自分の追記は別の見出しとして足す（§2）。
