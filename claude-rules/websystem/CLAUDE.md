...
- **このプロジェクト固有でなく、配布元の共通テンプレート全体に還元すべき**と判断した気づきは `[テンプレ還元]` と印を付ける。届け方は `rule-improvements.md`「`[テンプレ還元]` 印のものをテンプレ更新側へ届ける」参照（既定は棚卸し時にユーザーがまとめてテンプレ更新側へ持ち帰る／mailbox 配線済みなら `rule-kaizen` 宛に送ってよい）。固有値はテンプレに持ち込まず `project-config.md` 側に置く
