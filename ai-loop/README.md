...
```
.claude/rules/reviewer.md を読んで、Reviewer モードで動作して。
...
- git commit / push 禁止
- 詳細は `.claude/rules/reviewer.md` 参照

## トラブルシューティング

| 症状 | 対処 |
|------|------|
| Reviewer が conversation.md 以外を書こうとする | 即停止 → `.claude/rules/reviewer.md` の制約を再確認させる |
...
- `.claude/rules/reviewer.md` — Reviewer モードの詳細ルール
