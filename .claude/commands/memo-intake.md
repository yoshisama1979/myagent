...
5. 処理した各メッセージを `bin/.venv/bin/python3 bin/slack-poll.py done <msg_id>` で `cur/` へ移す（memo-stock/・new/ どちらからでも拾える）。回答を反映できた確認スレッドは `bin/.venv/bin/python3 bin/slack-poll.py untrack <thread_ts>` で追跡解除（未解決は残す）
