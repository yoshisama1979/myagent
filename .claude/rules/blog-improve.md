...
> **version 0.2**（2026-07-15）｜ changelog はファイル末尾。
...
- improve-log にこの B（元記事）の**改善版draft id** が無ければ `wp-draft.py post`（新規下書き作成）。**タイトル先頭に「【改善版｜元<元記事post id>】」**を付ける（例：`【改善版｜元2082】…`）。元記事と区別する（誤公開・取り違え防止）のに加え、**社長が投稿一覧で「どの記事の改善版か」を一目で判別できる**ようにするため（2026-07-15 社長要望）。`<元記事post id>` は Step 2 で `get` した元記事の post id。返ってきた改善版 id を improve-log に記録。**本番反映時はこの「【改善版｜元<id>】」プレフィックスを外して本来のSEOタイトルとして使う**（＝プレフィックスはレビュー用マーカー）。
...
WP には「**【改善版｜元<元記事post id>】<改善後タイトル>**」の形式（例：`【改善版｜元2082】…`）で `status=draft` 投稿。プレフィックスで元記事を判別（本番反映時に外す）。**元記事は不変**。
...
- [ ] タイトルを「**【改善版｜元<元記事post id>】**」形式にしたか？（社長が投稿一覧でどの記事の改善版か判別できるように）
...
| 0.2 | 2026-07-15 | 改善版下書きのタイトル規約を「**【改善版｜元<元記事post id>】<タイトル>**」に変更（社長要望＝投稿一覧で「どの記事の改善版か」が判別できない問題の解消）。Step 4・出力フォーマット・自己チェックに反映。プレフィックスはレビュー用マーカーで本番反映時に外す。ツール改修は不要（`wp-draft.py --title` で指定）。あわせて `wp-draft.py` に Gutenberg ブロック化を実装済（生HTML投稿時の「非推奨のクラシックブロック」警告＋レイアウト崩れの解消・2026-07-15） |
