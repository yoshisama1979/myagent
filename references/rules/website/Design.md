...
- **AI(Claude等)はコンパイルを実行しない**。`sass.bat` などのビルドコマンドは走らせない。コンパイルは **開発者側で手動で実施** する運用
  - `.scss` を編集したら、最後に「開発者側で `sass.bat` を実行してコンパイルしてください」と報告する
  - 動作確認(ブラウザでの表示確認)が必要な場合も、コンパイル待ちであることを明示する
...
- **コンパイル済みの `.css` ファイルを直接編集しない**(`.scss` を編集する)
- **`sass.bat` などのコンパイルコマンドを AI 側で実行しない**。コンパイルは開発者が手動で行う
