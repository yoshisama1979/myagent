...
| T-020? | `build-journey.py`（積み上げビューの自動生成） | 現状 `data/partner/milestones.md`（追記専用の台帳＝データ）から `site/business/journey.html`（ビュー）へは **AIが手書きで反映**している。月1回の手作業が続くと行の取りこぼし・二重記載が起きうる（今は journey 末尾の「反映済み: 〜日付」ウォーターマークだけが担保）。台帳を読んでHTMLを全生成すれば冪等になる | milestones.md の行フォーマット（`日付｜before→after｜枝｜出典` の固定形＝**既にパース可能**）／journey の見せ方が固まっていること | ⛔ 未着手（2026-07-24 社長メモ＝「どこかのタイミングで」。着手の目安＝節目100超 or 見せ方の確定。移行コストは小） |

## T-019: finance-snapshot.py（経理入力状況＋財務スナップショット）
- **日付**: 2026-07-24 ｜ **状態**: 🧪試作（手動検証済み・経営パートナーへの配線は社長合意待ち）
- **目的**: 「会話だけでなく数字の根拠を見てアドバイスする」ための数値ダイジェスト（2026-07-24 社長要望）。①経理入力の遅れ・費目欠落・未分類残高 ②入力済み期間ベースのPL ③BS（現預金・売掛金・集中度） ④案件シートの動き ⑤ストック収益。
- **使い方**: `python3 bin/finance-snapshot.py`（要約）／`--json`／`--no-fetch`（Sheets を叩かずキャッシュのみ＝速い）／`--date YYYY-MM-DD`（基準日）
- **設計原則（最重要）**: **期中PLの単純12倍予測を出さない**。費用の入力が遅れているとき、12倍予測は利益を極端に過大表示する（実測＝FY2026 の 2ヶ月PLで営業利益率86.5%＝人件費が丸ごと未入力なだけ）。代わりに「入力済み期間の実績」「前年同ペース比」「未入力費目を前年ペースで補った補正値」を出す。**会計（MF）と案件シートは基準が違うので合算・換算しない**。目標値は goals.html を読むだけ（二重台帳にしない）。
- **秘密の扱い**: BS の売掛金補助科目には**取引先名が入っている**が、名前は一切保持・出力しない（`data/financial/README.md`「個別取引明細・取引先名の詳細は集計結果から除外」）。依存度は名前を伏せた比率のみ。
- **限界**: 会計データは社長がMFクラウドからCSVを投入した時点のもの＝ツールは遅れを検知できるが埋められない／案件シートの品質アラート（請求漏れ疑い等）は**全期間の累計値**で期間で絞れない（上流 `projects-overview-data.py` が件数しか返さないため）＝棚卸しが要る／`--no-fetch` 以外は毎回 Sheets を叩く（30〜60秒）／**費目欠落の検知は「今期ゼロ」だけを見る**＝科目の改称・統合・廃止は誤検知しうるし、決算期にまとめ計上する費目（保険料・減価償却費・租税公課）も鳴る＝断定せず「入力漏れか年次計上か要確認」と出す。一部だけ入力された科目はゼロでないので拾えないため、**販管費合計の対前年ペース比**（70%未満で警報）で偽陰性を補っている／**MF CSV のパーサーが本ツール・`_financial-lib.php`・`analyze-mf.py` の3系統に分かれている**（Codex 2026-07-24 指摘・共通テストフィクスチャは未整備＝将来の統合候補）。
- **関連**: `bin/analyze-mf.py`（決算CSVの詳細分析）／`site/business/annual-forecast.php`（同じCSVを12倍予測で表示＝⚠️本ツールと結論が食い違う。要調整）／`bin/projects-overview-data.py`
