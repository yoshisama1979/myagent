...
## ⚠️ 呼び出し規約（全ツール共通・必ず先に読む・2026-07-26 制定）

**`bin/` のスクリプトは `bash ` を前置せず、登録形そのままで呼ぶ。**

```
✅ bin/hp-shot.sh <URL> <出力先> <名前>
❌ bash bin/hp-shot.sh <URL> <出力先> <名前>     ← 拒否される（別コマンド扱い）
```

- 実行許可リスト（`.claude/settings.local.json`）は **`Bash(bin/x.sh:*)` と `Bash(bash bin/x.sh:*)` を別コマンドとして扱う**。多くのツールは**前置なしの形だけ**が登録されている（`hp-audit.sh` は例外的に両形が登録済＝一番よく使う道具だけが寛容なため、`bash` を付ける癖が強化され、他ツールで静かに失敗する）。
- **拒否されたら「承認が下りない」と結論せず、前置を外してもう一度試す**。前置形だけを繰り返す再試行は検算にならない。
- **繰越を記録するときは「ツール名」でなく「実行形」の単位で書く**：❌「hp-shot は不可」／✅「`bash bin/hp-shot.sh` は不可・`bin/hp-shot.sh` は未検証」。前者は誤った制約を翌サイクルへ申し送ってしまう。
- 実害の記録：**yokohawaii 21サイクル（K-325b）→ yoshida 1サイクル（K-337）→ fujisaka 36日（K-339）＝3日連続・3サイトで同じ穴**。いずれも「network の承認が下りない」という**誤診**で、ビジュアル観察・ライブ実査が長期停止していた。
...
- **更新 2026-07-25（`--plumbing` 配管点検モードを追加・fujisaka Cycle 045）**：月次定石 #9「配管一斉点検」の **③解析タグ棚卸し ④JS棚卸し（フォーム検証JS・死にコード） ⑤混在コンテンツ（http:// リソース参照） ⑥CMS/ライブラリのバージョン露出** を1コマンドで出力（`bash bin/hp-audit.sh <URL> --plumbing`。HTTP GET のみ・読み取り専用・既存の要約/JSON/--text 出力は不変）。検出＝GTM/GA4(G-)/旧UA/Google広告(AW)/Clarity/Yahooタグ/FBピクセル＋**コメント内に残るタグ（残骸）**／外部script一覧・インライン数・ライブラリ痕跡（jquery・datepicker・slick 等）／`src`・`<link href>` の http:// 参照（＝ブラウザがブロックする混在コンテンツ。`<a href>` は別枠で参考表示）／generator・`?ver=`・WPプラグイン/テーマ名。**作った理由＝許可の壁を「新ツール新設」でなく「許可済みツールの拡張」で越えるため**：無人ループでは raw `curl`/`grep` が承認外で③〜⑥が2サイクル連続で繰越になっていたが、`Bash(bash bin/hp-audit.sh:*)` は既に allow 済み＝**新しい許可を1つも増やさずに当日実行できた**（新規スクリプトを作ると allowlist 追加＝社長作業が発生し、また待ちになる）。fujisaka Cycle 045 で実証＝/blog/ の**混在コンテンツ（http:// の Google Fonts CSS＝ブラウザにブロックされ無効）**・**WordPress 6.5.8 の generator 露出**・**/contact/ の jQuery UI 二重読み込み**を検出（R-F20/R-F21 起票）。**限界**：静的HTMLのみ（GTM 経由で後から注入されるタグは見えない＝「GA4 が直接見えない」は未設置の意味ではない）／1ページ単位。
- **更新 2026-07-27（alt 欠落・alt 空の `src` 列挙を追加・yoshida Cycle 046）**：`images` ブロックに **`no_alt_src` / `empty_alt_src`（各最大20件）** を追加し、要約出力にも `- alt欠落: <src>` の行を出すようにした（既存フィールドは不変＝hp-diff 後方互換）。`src` が無い遅延読み込み img は `data-src` / `data-original` / `srcset` を順に見て `値 (属性名)` の形で出す。**作った理由＝件数だけでは実装側が直せないから**：yoshida-dev から「/access/ に alt 欠落は 0件・具体的な src を教えてほしい」と認識合わせが来たが、**当方は件数しか持っておらず反証も特定もできなかった**（＝ツールが「指摘はできるが引き渡せない」形になっていた）。拡張の結果、その場で3件を特定＝<span>images/top/phone.png・calender.png・information.png</span> が**全ページ共通テンプレート**（トップ/access/member/jirei の4ページで同一）＝**ページ別の問題ではなく1箇所の修正で全ページ解消**、かつ**予約導線のアイコンそのもの**と分かった。あわせて `/jirei/` の「alt空4件」も装飾アイコンで**症例のbefore/after写真は該当しない**ことが分かり、**当方の誤った提案（R-Y11(c)）を撤回**できた。＝**「件数の集計は、指摘を反証可能にしない」**（実装側は src が無いと直せず、分析側は src が無いと自分の誤りにも気づけない）。**限界**：静的HTMLのみ（JS描画後に挿入される img は見えない）／上限20件で打ち切り（件数は `no_alt` / `empty_alt` が正）／`alt=""`（装飾として正しい空 alt）と「入れ忘れの空 alt」は区別できない＝人が判断する。
- **更新 2026-07-27（`--imgs` ＝ alt の「中身」列挙を追加・yoshida Cycle 047）**：`images` ブロックに **`alts`（`{src, alt}` の全件・最大80件。`alt` は属性が無ければ `null`・空文字なら `""`）** を追加し、要約出力では **`--imgs` を付けたときだけ** `· <src> → alt="…"` の行を出す（既定の出力は不変＝ノイズを増やさない。既存フィールドも不変＝hp-diff 後方互換）。**作った理由＝同じ日に同じ穴に2回落ちたから**：前項（同日の `no_alt_src` 追加）で「どの画像か」は言えるようになったが、**「alt はあるが非記述的」（例 `alt="治療前"`・`alt="1"`）は依然として検出も検証もできなかった**。実際 yoshida-dev が R-Y11(c) で「固定文言 alt → 記述的 alt に変更した」と報告してきたとき、当方の道具では **`no_alt`/`empty_alt` の件数が変わらないため、実装されたのかどうか本番GETで判定できなかった**（＝報告を鵜呑みにするしかない＝K-014「報告でなく本番URLで完了判定する」が道具の都合で守れない状態）。拡張の結果、その場で6枚すべての alt を実測し `alt="インプラント治療前｜症例1「50代　男性」"` を確認＝**✅完了と判定できた**。あわせて装飾アイコン `img/case/no1〜4.png` が `alt="1"`（意味のない連番）→ `alt=""` に適正化されたことも確認（`empty_alt` 4→8 の増加として現れる＝**「空 alt が増えた」が改善のサインになりうる**ので件数だけを見て退行と誤読しない）。＝**前項の教訓「件数の集計は指摘を反証可能にしない」の第二段＝「欠落の有無」の集計は"質"の改善を可視化しない**。**限界**：静的HTMLのみ／80件で打ち切り（件数は `total`/`no_alt`/`empty_alt` が正）／alt が記述的かどうかの**判断は人（AI）が行う**（ツールは中身を出すだけ・良し悪しは判定しない）。
...
- **🔧 改修 2026-07-26（hp-loop yoshida Cycle 045）＝既定CVイベントを dataset 別に**：`DEFAULT_CV_EVENTS` は **はなさか(ycom)向けの命名しか持たず**（問い合わせ・見積・tel 系）、よしだ歯科の北極星である **WEB予約 `reservation` を含んでいなかった**。そのため `landing`／`cv` を `--events` なしで叩くと **全ページの予約CVが 0 と表示される**（＝北極星が沈黙する。実際 Cycle 045 で既定のまま実行し、トップの CVR が 10.41% → 3.64% と別物の数字を得た）。対策＝`DATASET_CV_EVENTS`（dataset → イベント名リスト）と `cv_events_for()` を追加し、**優先順＝`--events` 明示 > dataset 別既定 > 全体既定**。あわせて**テキスト出力のヘッダに「CV対象イベント: …」を必ず印字**（何を CV と数えたかを毎回目視できる）。登録済＝`analytics_287348176`（yoshida＝`reservation`／`インターネットによるご予約`／`tel`／`スマホからの電話問い合わせ`）。**回帰確認済**＝ycom（`analytics_265729912`）は従来どおり `DEFAULT_CV_EVENTS`。**新サイトを GA4 で測るときは、まず `events` モードでイベント名を実査し、この表に登録してから `landing`／`cv` を使うこと**（既定のまま使うと静かにゼロが出る）。

---

## T-021 — `bin/hp-links.py`（ページのリンク棚卸し＝アンカー文言・リンク先・死活）

- 2026-07-25 / 🟡**作成済・未実行（許可リスト未登録）** ｜ 契機＝hp-loop yokohawaii Cycle 032
- **目的**：`hp-audit.sh` はリンクを**本数でしか出さない**（`links: {total, tel, mailto, contact}`）ため、「**どのアンカー文言で・どこへ**」が分からない。内部リンク設計の検証（文脈内リンクの有無）・発リンク元としての価値・死にリンクの検出に必要。
- **契機となった具体的な行き詰まり**（3件が同じ不足に当たった）：①R-K13＝「`/features/` から `/condominium/` への内部リンクが無い」と18サイクル書いていたが、実際は**ナビ・フッタに存在**し欠落は「本文中の文脈内リンク」だった（本数だけ見て中身を見ていなかった）②`/blog/` から `http://www.yoko-hawaii.com/sp/` へのリンク3本が **404**（廃止済みサイトの残骸）③`/condominium/` の「コンドミニアム詳細」リンク先が実在するか**未検証のまま**（R-K07 の前提が確定できない）。
- **使い方**：`bin/.venv/bin/python3 bin/hp-links.py <URL>`／`--check`（内部リンクのHTTPステータスを HEAD で確認・非200を ⚠️ で集約）／`--filter <部分文字列>`（リンク先で絞り込み）
- **出力**：`[内|外] アンカー文言 -> 絶対URL ×重複数 [HTTP nnn]`。グローバルナビ等で膨らむ重複は件数に畳む。HTMLコメント内のリンクは除外（クローラが辿らないため）。
- **設計**：HTTP GET / HEAD のみ・**読み取り専用**（外部送信・ファイル書き込み・破壊的操作なし＝automation.md 準拠）。`--check` は**同一ホストの内部リンクだけ**を対象（他所のサーバを叩かない）。取得失敗は成功を装わず `ERROR` で落とす。
- **🔴 現状の状態＝実行できない**：`.claude/settings.local.json` の allow に未登録のため、作成した当サイクルでは**1回も実行できていない**（出力は未取得＝掲示板にも「未検証」と明記した）。**登録が要る形**＝`Bash(bin/.venv/bin/python3 bin/hp-links.py:*)`。**2026-07-26（hp-loop yoshida Cycle 045）で2回目の実行不能を確認**＝トップのキャンペーンバナーのリンク先を調べようとして弾かれ、リンク先不明のまま提案を書くことになった（作ったのに一度も使えていないツールが在庫化している）。**T-001 の教訓（2026-06-20）と同じく、`bash ` 前置形とそうでない形は別コマンド扱い**になるため、無人実行で使うなら呼び出し形を固定して登録すること。
- **限界**：静的HTML のみ（JS で描画されるリンクは見えない＝T-001 と同じ）／1ページ単位（サイト全体クロールはしない）／`--check` は HEAD を返さないサーバで誤判定しうる／リンクの**位置**（ナビ／本文／フッタ）は判別しない＝「文脈内リンクか」の最終判断は `--text` と併読する必要がある（将来の改善候補）。

---

## T-022 — `bin/ads-fetch.py`（Google広告＝BigQuery Data Transfer の読み取りフェッチャ）

- **作成**：2026-07-27（社長指示＝「レポート機能として AdWords も対応したい。MCC のデータを BigQuery に書き出しているので解析とつなげたい」）。gsc-fetch.py（T-006）／ga4-fetch.py（T-007）と同じ設計の第3の一次データ源。
- **データ源**：`myservice-219202.YCOM_MCC`（Google Ads Data Transfer・MCCレベル転送・**MCC ID = 7979056885 がテーブルサフィックス**・配下アカウントは行の `customer_id` 列で区別）。認証は**既存の GSC 用サービスアカウントがそのまま通る**（.env の `ADS_SA_JSON` 優先→`GSC_SA_JSON` フォールバック・新しい秘密情報なし）。
- **構造の実測確認（2026-07-27）**：①stats 表（`p_ads_*Stats_*`）は**パーティション日＝segments_date で1日1回格納**＝期間フィルタ付き単純SUMで二重計上しない（segments_date=2026-07-10 が partition 2026-07-10 のみに存在することを確認）②ディメンション表（Campaign 等）は日次スナップショット＝`ads_*` ビューの `_DATA_DATE = _LATEST_DATE` で最新だけ読む③データセットには旧AdWords転送世代（無印・`p_*`）と手作り集計（`my_*`）が同居するが**使うのは `p_ads_*`／`ads_*` のみ**④転送は毎日稼働・前日分まで（鮮度実測＝7/27時点で7/26まで）。
- **使い方**：`bin/.venv/bin/python bin/ads-fetch.py <mode> [--account CID]`。mode＝`accounts`（MCC内一覧）／`summary`／`daily`／`campaigns`（名前・予算つき）／`queries`（検索語。`--zero-conv` で CV0×費用あり＝無駄配信候補だけ）／`conversions`（**CVアクション別内訳＝「何をCVと数えているか」の検査**）／`paid-organic`（広告×自然検索の重なり）／`overview`（まとめてJSON）。共通：`--days/--start/--end/--limit/--dataset/--max-gb/--dry-run/--json`。
- **安全**：SELECT のみ・読み取り専用。日付＋アカウントはクエリパラメータで渡し文字列補間しない（SQLi対策）。識別子（project/dataset/サフィックス）は補間前に正規表現検証。`maximum_bytes_billed` 上限（既定2GB・実測スキャンはMB級＝dry-run 0.0GB）。
- **単独アカウント対応（将来）**：MCC 外のクライアントは、そのアカウント用 transfer を**クライアント別データセット**に作れば `--dataset` を変えるだけで同じツールで読める（サフィックスは自動検出）。
- **初回の実測成果（2026-07-27・28日窓）**：①**YCOM広告の「CV 779件・CPA ¥25」の中身は8割超がスクロール計測**（`TOPページスクロールダウン70%`＝516件・PAGE_VIEW category ほか）＝実アクション（料金クリック）は2.5件・実質CPA約¥7,900。見込み客診断（hikita）で見つけた「ページ閲覧でCV発火」と**同じ病気が自社にもある**ことが即日判明②よしだ歯科は CVアクションが「GA4 インターネットによるご予約」の実CVで健全（73.8件・CPA ¥793）③ソリデンテ＝30日 ¥27,061・495クリックで **CV 0**（計測破損か実ゼロか要確認）④paid-organic は YCOM で空＝広告⇔GSCのアカウントリンク未設定だったため（**2026-07-27 社長がリンク実施済み〔ycom・yoshida〕**＝データはリンク日以降から。UIは新レイアウト「ツール→データ マネージャー→Search Console」＝旧「リンク アカウント」は廃止）。
- **限界**：検索語レポートはGoogle側のプライバシー閾値で一部の語が出ない／conv_value の意味はアカウントのCV値設定次第（よしだ歯科は1予約=¥10,000 換算らしき値）／MCC の総管理費用は見えるが請求額（クレジット等調整後）とは別物／金額表示は¥固定（現状全アカウントJPY・通貨コードは accounts モードで確認可＝多通貨アカウントが入ったら要改修）／転送遅延の機械フラグ化（summary の last_date≠期間末を JSON に明示するフィールド）は将来改修（現状はレシピ#4c の読み方の規律で対応）。
- **Codexレビュー（2026-07-27・コミット前）**：🔴3件🟡2件を反映＝①accounts 以外 --account 必須化（**省略時に daily/queries/conversions が customer_id で分けず集計＝別アカウントの数字が混ざったまま正常終了する穴**）②片側だけの --start/--end を黙って無視・--days 0 以下が通る入力検証の穴③last-run 境界「超→以上」統一（ルール側）④match_type の ANY_VALUE 不定表示⑤転送遅延の誤検知防止規律。指摘なしと明言＝二重計上・JOIN膨れ・SQLi・課金上限・秘密情報・例外時の無言成功。不採用1件＝未追跡ファイル参照のリンク切れ（クライアント分析git外方針による意図的状態＝config.md に注記で対応）。
- **hp-loop 組み込み（2026-07-27 社長承認「1から進めましょう」で実施済み）**：①`sites/ycom.md`・`sites/yoshida.md` に「広告アカウント」行（customer_id・初回実測の要点つき）②`config.md` データソース表に Google広告 行③`rules/modes/hp-loop.md` **v0.15**＝広告定石（#4c 日次異常検知・#11 週次無駄刈り＋CV健全性）＋last-run 周期判定を「周期」列ベースに変更④ycom 台帳 §8/8a に #4c・#11 登録（初回実施日 2026-07-27 済）⑤許可リスト登録＝`Bash(bin/.venv/bin/python3 bin/ads-fetch.py:*)`＋`python` 形の2形（T-021 の「作ったのに実行できない」再発防止・python3 形で実行確認済み）。yoshida の台帳は未整備のため、台帳整備時に同レシピ表を持つ（それまではルール＋sites config が根拠）。
