...
| T-013? | gsc-fetch にクエリ→着地ページ逆引き | 「高順位0クリック」クエリ群の受け皿ページを1コマンドで特定（K-197。現状は pages 側集計からの手作業逆引き＝M-04 で運用回避中） | gsc-fetch.py 拡張（query 指定で page 別分解） | ⛔ 未着手（2026-07-08 棚卸しで転記） |
